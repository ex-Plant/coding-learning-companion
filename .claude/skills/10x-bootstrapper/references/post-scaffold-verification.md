...
**Package-manager-aware dispatch.** When the value at `audit_commands[<language_family>]` is a map (not a string), index further by the resolved `package_manager` (the hand-off value, or the card's `toolchain.package_manager` fallback). If the resolved pm key is missing from the map, fall back to the map's `_default` entry; if no `_default` is present, log "audit command unmapped for <pm>" and skip the audit (treat as the null branch). Rationale: `npm audit` requires `package-lock.json` and fails with `ENOLOCK` on a pnpm-only project; `pnpm audit` is the right tool when the user picked pnpm. Same logic applies to other multi-pm ecosystems (yarn, bun).
