...
- `{name}` — replaced by `bootstrap-scaffold` for `subdir-then-move` and `git-clone`; replaced by `.` for `native-cwd`. The name has no leading dot because some CLIs (notably `create-next-app`) refuse names starting with `.` per npm naming restrictions. The temp directory is deleted at the end of Step 2 regardless of its name, so visibility is not a concern.
