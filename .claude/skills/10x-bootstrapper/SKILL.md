...
Look up `cwd_strategy` for the chosen `starter_id` from `references/bootstrapper-config.yaml` now (default to `subdir-then-move` if the id is not listed). Step 2 needs it. Look up `audit_commands[<hints.language_family>]` from the same file at the same time and stage it for Step 3. If the value is a map (multi-pm ecosystem — e.g. `js`), index further by the resolved `package_manager`; fall back to the map's `_default` key if the pm is absent. A `null` value (or an unresolvable map lookup with no `_default`) means Step 3 will skip the audit and note the skip in the log.
